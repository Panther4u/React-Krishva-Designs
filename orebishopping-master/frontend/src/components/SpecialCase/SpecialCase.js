...
const SpecialCase = () => {
  ...
  return (
    <div className="fixed top-52 right-2 z-20  md:flex flex-col gap-2">
      ...
      {/* <Link to="/cart">
        <div className="bg-white w-16 h-[70px] rounded-md flex flex-col gap-1 text-[#33475b] justify-center items-center shadow-testShadow overflow-x-hidden group cursor-pointer relative" style={{backgroundColor : '#128C7E'} }>
          <div className="flex justify-center items-center">
          ...
          <p className="text-xs font-semibold font-titleFont text-center text-white">Chat</p>
          {products.length > 0 && (
            <p className="absolute top-1 right-2 bg-primeColor text-white text-xs w-4 h-4 rounded-full flex items-center justify-center font-semibold">
              {products.length}
            </p>
